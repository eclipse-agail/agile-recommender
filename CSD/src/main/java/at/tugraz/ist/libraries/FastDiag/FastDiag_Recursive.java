package at.tugraz.ist.libraries.FastDiag;
...
public class FastDiag_Recursive {
    ...
    private static CSP masterCSP; 
    static Constraints_Singleton inst = Constraints_Singleton.getInstance();
     ...
     */
    public static List<Constraint> computeDiagnose(List<Constraint> C, List<Constraint> AC, CSP solver_csp) {
    	
    	masterCSP = new CSP(false,null,solver_csp,null,0,0);
...
//        		System.out.println("VAL: "+Constraints_Singleton.getInstance().getConstraintList_extension__UserRequirements().get(cID).getValue_1());
         ...
         */
        solver_csp.chocoModel.getSolver().reset();
        Boolean foundSolution = solver_csp.chocoModel.getSolver().solve();
        if (!foundSolution) {
            return new ArrayList<Constraint>(); // empty list is equal to zero;
        } else {
            /*
    ...
    private static List<Constraint> FD(List<Constraint> D, List<Constraint> C, List<Constraint> AC) {
        ...
        if (!D.isEmpty()) {
        	...
        	CSP tempCSP = new CSP(false,null,masterCSP,null,0,0);
        	Solver solver  = tempCSP.chocoModel.getSolver();
